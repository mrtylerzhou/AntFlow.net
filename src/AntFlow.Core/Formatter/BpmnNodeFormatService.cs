...
public class BpmnNodeFormatService
{
        ...
        BpmnStartConditionsVo bpmnStartConditions)
    {
        ...
        List<BpmnConfCommonElementVo> bpmnConfCommonElementVos = new();
        ...
        List<BpmnNodeVo> rebuildNodesList = new();
        ...
        Dictionary<string, string> singleAssigneeMap = new() { { startUserId, bpmnStartConditions.StartUserName } };
        ...
        {
            { "nodeCode", nodeCode.Value }, { "sequenceFlowNum", sequenceFlowNum }
        };
        ...
        {
            FormatNodes(bpmnConfCommonElementVos, rebuildNodesList, startUserNodeTo, nodeCode.Value, sequenceFlowNum,
                numMap);
        }
        ...
        BpmnConfCommonElementVo? lastSequenceFlow =
        ...
        Dictionary<string, int> numMap)
    {
        ...
        Dictionary<string, BpmnNodeVo>? mapNodes = rebuildNodesList.ToDictionary(
            node => node.NodeId,
            node => node,
    ...
    {
        Dictionary<string, BpmnNodeVo>? mapNodes = nodes.ToDictionary(node => node.NodeId, node => node);
        string nextId = null;
        ...
        do
        {
            ...
            if (nextParams == null ||
                (string.IsNullOrEmpty(nodeTo) && (int)NodeTypeEnum.NODE_TYPE_COPY == nextNodeVo.NodeType))
            {
                return;
            }
                ...
                if ((int)NodeTypeEnum.NODE_TYPE_PARALLEL_GATEWAY == vo.NodeType)
                {
                    ...
                    if (aggregationNode == null)
                    {
                        throw new AFBizException(
                            "there is a parallel gateway node,but can not get its aggregation node");
                    }
