...
public class ConditionFilterService
{
    ...
    public void ConditionfilterNode(BpmnConfVo bpmnConfVo, BpmnStartConditionsVo bpmnStartConditionsVo)
    {
        ...
        Dictionary<string, BpmnNodeVo> nodeIdMapNode = new(16);
    ...
    {
        List<BpmnNodeVo> nodeList = new();
        BpmnNodeParamsVo parameters = new();
        parameters.NodeTo = startNode.NodeTo[0];
        startNode.Params = parameters;
        string nextId = parameters.NodeTo;
        do
        {
            if ((int)NodeTypeEnum.NODE_TYPE_GATEWAY == startNode.NodeType && startNode.IsParallel.HasValue &&
                startNode.IsParallel.Value)
            {
        ...
        BpmnStartConditionsVo bpmnStartConditionsVo)
    {
        ...
        {
            if (!nodeIdMapNode.TryGetValue(nodeTo, out BpmnNodeVo? currentNodeVo))
            {
                continue;
            }
            ...
            for (BpmnNodeVo? nodeVo = currentNodeVo;
                 ...
                 nodeVo = nodeIdMapNode.TryGetValue(nodeVo.Params.NodeTo, out BpmnNodeVo? nextNode) ? nextNode : null)
            {
        ...
        List<BpmnNodeVo> filterNodeList, BpmnStartConditionsVo bpmnStartConditionsVo)
    {
        ...
        {
            List<BpmnNodeVo> beforeFilterNodeList = new();
    ...
    {
        Dictionary<string, BpmnNodeVo>? notConditionNodeMap = new();
        Dictionary<string, BpmnNodeVo>? conditionNodeMap = new();

        foreach (BpmnNodeVo? node in nodeList)
        {
            if (node.NodeType == (int)NodeTypeEnum.NODE_TYPE_GATEWAY ||
                node.NodeType == (int)NodeTypeEnum.NODE_TYPE_CONDITIONS)
            {
        ...
        foreach (KeyValuePair<string, BpmnNodeVo> entry in notConditionNodeMap)
        {
            BpmnNodeVo? currentNode = entry.Value;
            string? nextNodeId = currentNode.Params?.NodeTo;
            ...
            if (!notConditionNodeMap.ContainsKey(nextNodeId))
            {
                ...
                if (!string.IsNullOrEmpty(resultNodeId))
                {
                    ...
                    if (!string.IsNullOrEmpty(bpmnNodeVo.NodeFrom) &&
                        notConditionNodeMap.ContainsKey(bpmnNodeVo.NodeFrom) &&
                        notConditionNodeMap[bpmnNodeVo.NodeFrom].NodeType !=
                        (int)NodeTypeEnum.NODE_TYPE_PARALLEL_GATEWAY)
                    {
    ...
    private string FindNext(string nodeId, List<string> addNodeIdList,
        Dictionary<string, BpmnNodeVo> notConditionNodeMap, Dictionary<string, BpmnNodeVo> conditionNodeMap)
    {
        ...
        if (!conditionNodeMap.TryGetValue(nodeId, out BpmnNodeVo? bpmnNodeVo))
        {
    ...
    private List<BpmnNodeVo> FilterParallelCondition(List<BpmnNodeVo> beforeFilterNodeList,
        BpmnStartConditionsVo bpmnStartConditionsVo)
    {
        ...
        List<BpmnNodeVo> filteredNodes = new();
