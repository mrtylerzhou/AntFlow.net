...
public class BpmnSendMessageAspect<T> : DispatchProxy
{
    ...
    protected override object Invoke(MethodInfo targetMethod, object[] args)
    {
        ...
        if (businessPartyId != null && businessPartyId > 0)
        {
            ...
            outSideBpmBusinessParty =
                outSideBpmBusinessPartyService.baseRepo.Where(a => a.Id == businessPartyId).First();
            //set outside type
        ...
        if (businessDataVo.OperationType == (int)ProcessOperationEnum.BUTTON_TYPE_SUBMIT)
        {
            ...
            DoMethod(bpmnConf, businessDataVo, outSideBpmBusinessParty, ProcessOperationEnum.BUTTON_TYPE_SUBMIT,
                targetMethod, args);
            return null;
        }

        BpmVariableMessageService bpmVariableMessageService =
            ServiceProviderUtils.GetService<BpmVariableMessageService>();
        ...
        ProcessOperationEnum? processOperationEunm =
            ProcessOperationEnumExtensions.GetEnumByCode(businessDataVo.OperationType);
     ...
     * @throws Throwable
     */
    private void DoMethod(BpmnConf bpmnConf, BusinessDataVo businessDataVo,
        OutSideBpmBusinessParty outSideBpmBusinessParty,
        ProcessOperationEnum processOperationEunm, MethodInfo targetMethod, object[] args)
    {
        ...
        if (bpmnConf.IsOutSideProcess == 1 && !businessDataVo.IsOutSideChecked)
        {
            ...
            OutSideBpmCallbackUrlConfService outSideBpmCallbackUrlConfService =
                ServiceProviderUtils.GetService<OutSideBpmCallbackUrlConfService>();
            //query and set process flow callback url
            OutSideBpmCallbackUrlConf outSideBpmCallbackUrlConf =
                outSideBpmCallbackUrlConfService.GetOutSideBpmCallbackUrlConf(bpmnConf.BusinessPartyId ?? 0);
    ...
    {
        object proxy = Create<T, BpmnSendMessageAspect<T>>();
