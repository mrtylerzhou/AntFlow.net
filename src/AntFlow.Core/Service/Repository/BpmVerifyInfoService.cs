...
public class BpmVerifyInfoService : AFBaseCurdRepositoryService<BpmVerifyInfo>, IBpmVerifyInfoService
{
    ...
    {
        BpmFlowrunEntrust entrustByTaskId =
            _bpmFlowrunEntrustService.GetEntrustByTaskId(SecurityUtils.GetLogInEmpIdStr(), verifyInfo.RunInfoId,
                verifyInfo.TaskId);
    ...
    public string FindCurrentNodeIds(string processNumber)
    {
        ...
        List<string>? bpmnNodeIds = _bpmVariableService.GetNodeIdsByeElementId(processNumber, elementId);
        ...
        List<string>? nodeCollect = bpmnNodes.Select(node => node.NodeId).ToList();
    ...
    public List<BpmVerifyInfoVo> FindTaskInfo(BpmBusinessProcess bpmBusinessProcess)
    {
        ...
        foreach (BpmVerifyInfoVo? task in tasks)
        {
        ...
        List<BpmVerifyInfoVo> taskInfors = new();
    ...
    {
        BpmVerifyInfoVo vo = new() { ProcessCode = processNumber };
    ...
    {
        List<BpmVerifyInfoVo>? infoVoList = new();
            ...
            if (!string.IsNullOrEmpty(o.OriginalId))
            {
                ...
                else
                {
                    Dictionary<string, string>? employeeInfo =
                        _bpmnEmployeeInfoProviderService.ProvideEmployeeInfo(new List<string> { o.OriginalId });
                    if (employeeInfo.TryGetValue(o.OriginalId, out string? originalName))
                    {
