...
public class NodePropertyLoopAdaptor : BpmnNodeAdaptor
{
    ...
    public override BpmnNodeVo FormatToBpmnNodeVo(BpmnNodeVo bpmnNodeVo)
    {
        ...
        if (bpmnNodeLoopConf != null)
        {
            ...
            List<BaseIdTranStruVo>? loopEndPersonList = _bpmnEmployeeInfoProviderService
                .ProvideEmployeeInfo(loopEndPersonIds)
                .Select(entry => new BaseIdTranStruVo { Id = entry.Key, Name = entry.Value }).ToList();

            List<BaseIdTranStruVo>? noParticipatingStaffList = _bpmnEmployeeInfoProviderService
                .ProvideEmployeeInfo(noParticipatingStaffIds)
                .Select(entry => new BaseIdTranStruVo { Id = entry.Key, Name = entry.Value }).ToList();
    ...
    public override void EditBpmnNode(BpmnNodeVo bpmnNodeVo)
    {
        ...
        BpmnNodeLoopConf? bpmnNodeLoopConf = new()
        {
