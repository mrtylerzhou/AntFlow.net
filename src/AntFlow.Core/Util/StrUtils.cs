...
public static class StrUtils
{
    ...
    public static string JoinBpmnCode(string bpmncodePart, string bpmnCode)
    {
        ...
        {
            string[]? parts = bpmnCode.Split(new[] { StringConstants.BPMN_CODE_SPLITMARK }, StringSplitOptions.None);
            maxNumStr = parts[^1];
        }
        ...
        string stringWithRawFormat =
            bpmncodePart + StringConstants.BPMN_CODE_SPLITMARK + "{" + BPMNCONF_FORMATMARK + "}";
    ...
    public static string[] GetNullPropertyNames(object source)
    {
        ...
        string[]? propertyNames = TypeDescriptor.GetProperties(source)
    ...
    private static string GetFirstPinYin(string hanyu)
    {
        ...
        StringBuilder? firstPinyin = new();
        char[] hanyuArr = hanyu.Trim().ToCharArray();

        foreach (char ch in hanyuArr)
        {
