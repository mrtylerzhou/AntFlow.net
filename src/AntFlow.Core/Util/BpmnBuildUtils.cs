...
public static class BpmnBuildUtils
{
    ...
    {
        return new StartEvent { Id = startEventId, Name = startEventName };
    }

    public static UserTask CreateUserTask(string id, string name, string assigneeParamName)
    {
        UserTask? userTask = new() { Id = id, Name = name, Assignee = FormatParamName(assigneeParamName) };
    ...
    public static UserTask CreateOrSignUserTask(string id, string name, string collectionName,
        string elementVariableName)
    {
        return CreateMultiplayerUserTask(id, name, collectionName, elementVariableName, true);
    }

    private static UserTask CreateMultiplayerUserTask(string id, string name, string collectionName,
        string elementVariableName, bool isOrSign)
    {
    ...
    public static UserTask CreateLoopUserTask(string id, string name, string assigneeParamName, string endLoopMark)
    {
        UserTask? userTask = new()
        {
            ...
            LoopCharacteristics = new MultiInstanceLoopCharacteristics
            {
                Sequential = true, LoopCardinality = "10", CompletionCondition = FormatParamName(endLoopMark)
            },
    ...
    public static UserTask CreateLoopUserTask(string id, string name, string loopCardinality, string collectionName,
        string elementVariableName)
    {
        UserTask? userTask = new()
        {
    ...
    {
        return new EndEvent { Id = endEventId, Name = endEventName };
    }
    ...
    private static AFActivitiListener GetActivitiListener(string eventName, string implementation)
    {
        ...
        {
            Evt = eventName, ImplementationType = "delegateExpression", Implementation = implementation
        };
    }
