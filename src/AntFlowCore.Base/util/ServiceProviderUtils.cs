...
public static class ServiceProviderUtils
{
    ...
    /// <summary>
    /// 获取所有实现了指定开放泛型类型的服务实例。
    /// </summary>
    /// <param name="openGenericType">开放泛型类型，如 typeof(IFormOperationAdaptor&lt;&gt;)</param>
    /// <returns>所有匹配的服务实例集合。</returns>
    public static IEnumerable GetServicesByOpenGenericType(Type openGenericType)
    {
        if (_serviceCollection == null) yield break;
