...
namespace AntFlowCore.Base.vo
{
    ...
    public static class NodeLabelConstants
    {
        ...
        /// 对应 Java NodeLabelConstants.conditionAutoTransferNode. 不加入 NONE_OPERATIONAL_NODES.
        /// </summary>
        public static readonly BpmnNodeLabelVO ConditionAutoTransferNode = new BpmnNodeLabelVO(StringConstants.CONDITION_AUTO_TRANSFER_NODE, "条件自动转办节点");
        ...
        /// <summary>条件退回节点:满足条件时自动退回到不同意按钮配置的目标节点,不满足时留给真实审批人.</summary>
        public static readonly BpmnNodeLabelVO ConditionReturnNode = new BpmnNodeLabelVO(StringConstants.CONDITION_RETURN_NODE, "条件退回节点");
        /// <summary>条件退回发起人节点:满足条件时自动退回发起人节点,不满足时留给真实审批人.</summary>
        public static readonly BpmnNodeLabelVO ConditionReturnStarterNode = new BpmnNodeLabelVO(StringConstants.CONDITION_RETURN_STARTER_NODE, "条件退回发起人节点");
        ...
        /// </summary>
        private static readonly List<string> AutoConditionNodeLabels = new List<string>
        {
        ...
        /// <summary>
        /// 是否自动/条件语义节点(按标签判断): 是则流程构建时强制 DeduplicationExclude = true.
        /// </summary>
        public static bool IsAutoConditionNode(List<BpmnNodeLabelVO> nodeLabelVOs)
        {
            if (nodeLabelVOs == null || nodeLabelVOs.Count == 0)
            {
                return false;
            }
            foreach (var label in nodeLabelVOs)
            {
                if (label != null && AutoConditionNodeLabels.Contains(label.LabelValue))
                {
                    return true;
                }
            }
