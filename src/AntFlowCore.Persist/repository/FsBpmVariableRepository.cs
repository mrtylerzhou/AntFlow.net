...
public class FsBpmVariableRepository : RepositoryBase<BpmVariable>, IBpmVariableRepository
{
    ...
    {
        List<string> nodeIds = _ormContext.FreeSql.Select<BpmVariable, BpmVariableMultiplayer>()
    ...
    {
        List<string> elementIds = _ormContext.FreeSql.Select<BpmVariable, BpmVariableMultiplayer>()
            .InnerJoin((a, b) => a.Id == b.VariableId)
    ...
    public void InvalidNodeAssignees(List<string> assigneeIds, string processNumber, bool isSingle)
    {
        ...
        if (isSingle)
        {
            // BpmVariableSignUp entity deleted, single node invalidation skipped
            return;
        }
